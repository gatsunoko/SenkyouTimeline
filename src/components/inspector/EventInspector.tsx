import { certaintyLabels, eventTypeLabels } from "../../data/pieceTemplates";
import { useProjectStore } from "../../store/projectStore";
import type { Certainty, EventType } from "../../types/project";
...
export function EventInspector({ id }: { id: string }) {
  ...
  return (
    <aside className="right-inspector">
      ...
      <TextField label="日付キー" value={event.time} onChange={(value) => updateEvent(event.id, { time: value })} />
      <TextField label="表示日付" value={event.displayDate} onChange={(value) => updateEvent(event.id, { displayDate: value })} />
      ...
      <SelectField<Certainty> label="確度" value={event.certainty} options={certaintyLabels} onChange={(value) => updateEvent(event.id, { certainty: value })} />
      ...
      <TextAreaField label="史料メモ" value={event.sourceNote} onChange={(value) => updateEvent(event.id, { sourceNote: value })} />
    </aside>
  );
}
