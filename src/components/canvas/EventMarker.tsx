...
export function EventMarker({ event, selected, onSelect }: EventMarkerProps) {
  const position = relativeToCanvas(event);
  return (
    <Group x={position.x} y={position.y} visible={event.visible} opacity={event.certainty === "uncertain" ? 0.55 : 0.95} onClick={onSelect} onTap={onSelect}>
